...
impl Database {
    ...
    pub fn save_collection(&self, collection: &Collection) -> ApiResult<()> {
        let auth_json = collection.auth.as_ref()
    ...
    fn save_folder(&self, folder: &CollectionFolder, collection_id: &str, parent_id: Option<&str>) -> ApiResult<()> {
        self.conn.execute(
    ...
    fn get_folders_by_collection(&self, collection_id: &str, parent_id: Option<&str>) -> ApiResult<Vec<CollectionFolder>> {
        let mut stmt = self.conn.prepare(
        ...
        let rows = stmt.query_map(
            params![collection_id, parent_param],
            |row| {
    ...
    pub fn list_collections(&self) -> ApiResult<Vec<Collection>> {
        let mut stmt = self.conn.prepare(
            "SELECT id FROM collections ORDER BY updated_at DESC"
        ).map_err(|e| ApiError::Database(e.to_string()))?;
    ...
    // Request operations
    pub fn save_request(&self, request: &ApiRequest, collection_id: Option<&str>, folder_id: Option<&str>) -> ApiResult<()> {
    ...
    pub fn get_request(&self, id: &str) -> ApiResult<Option<ApiRequest>> {
        ...
        let row = stmt.query_row([id], |row| {
            ...
            let method: HttpMethod = serde_json::from_str(&method_str)
                .map_err(|e| rusqlite::Error::FromSqlConversionFailure(0, rusqlite::types::Type::Text, Box::new(e)))?;
            let url: String = row.get(3)?;

            let headers: Vec<KeyValue> = serde_json::from_str(&row.get::<_, String>(4)?)
                .map_err(|e| rusqlite::Error::FromSqlConversionFailure(0, rusqlite::types::Type::Text, Box::new(e)))?;
            let query_params: Vec<KeyValue> = serde_json::from_str(&row.get::<_, String>(5)?)
                .map_err(|e| rusqlite::Error::FromSqlConversionFailure(0, rusqlite::types::Type::Text, Box::new(e)))?;
            let auth: Auth = serde_json::from_str(&row.get::<_, String>(6)?)
                .map_err(|e| rusqlite::Error::FromSqlConversionFailure(0, rusqlite::types::Type::Text, Box::new(e)))?;
            let body: Body = serde_json::from_str(&row.get::<_, String>(7)?)
                .map_err(|e| rusqlite::Error::FromSqlConversionFailure(0, rusqlite::types::Type::Text, Box::new(e)))?;
    ...
    fn get_requests_by_collection(&self, collection_id: &str, folder_id: Option<&str>) -> ApiResult<Vec<ApiRequest>> {
        let mut stmt = self.conn.prepare(
        ...
        let ids: Vec<String> = stmt.query_map(
            params![collection_id, folder_param],
            |row| row.get(0)
        ).map_err(|e| ApiError::Database(e.to_string()))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| ApiError::Database(e.to_string()))?;
    ...
    fn get_environment_variables(&self, env_id: &str) -> ApiResult<Vec<EnvironmentVariable>> {
        ...
        let vars = stmt.query_map([env_id], |row| {
    ...
    pub fn list_environments(&self) -> ApiResult<Vec<Environment>> {
        let mut stmt = self.conn.prepare(
            "SELECT id FROM environments ORDER BY is_default DESC, name ASC"
        ).map_err(|e| ApiError::Database(e.to_string()))?;
    ...
    pub fn get_history(&self, limit: usize) -> ApiResult<Vec<HistoryEntry>> {
        ...
        let entries = stmt.query_map([limit], |row| {
            let id: String = row.get(0)?;
            let request: ApiRequest = serde_json::from_str(&row.get::<_, String>(1)?)
                .map_err(|e| rusqlite::Error::FromSqlConversionFailure(0, rusqlite::types::Type::Text, Box::new(e)))?;

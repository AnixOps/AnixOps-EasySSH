...
impl SftpSessionManager {
    ...
    /// 重命名
    pub async fn rename(&self, session_id: &str, old_path: &str, new_path: &str) -> Result<(), LiteError> {
        let sftp_mutex = self.sessions
            .get(session_id)
            .ok_or(LiteError::Ssh("SFTP会话不存在".to_string()))?;
    ...
    /// 下载文件
    pub async fn download(&self, session_id: &str, remote_path: &str) -> Result<Vec<u8>, LiteError> {
        let sftp_mutex = self.sessions
            .get(session_id)
            .ok_or(LiteError::Ssh("SFTP会话不存在".to_string()))?;
    ...
    /// 上传文件
    pub async fn upload(&self, session_id: &str, remote_path: &str, contents: &[u8]) -> Result<(), LiteError> {
        let sftp_mutex = self.sessions
            .get(session_id)
            .ok_or(LiteError::Ssh("SFTP会话不存在".to_string()))?;
...
impl SftpEntry {
    ...
    pub fn mtime_display(&self) -> String {
        if self.mtime == 0 {
            ...
            let dt = chrono::DateTime::from_timestamp(self.mtime, 0)

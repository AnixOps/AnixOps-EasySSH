...
pub fn show_edit_server_dialog<F>(parent: &adw::ApplicationWindow, server: &Server, callback: F)
where
    F: FnOnce(Server) + 'static,
{
    ...
    let auth_selected = match server.auth_type {
        AuthType::Password => 0,
        AuthType::Key => 1,
        AuthType::Agent => 2,
    };
    ...
    let password_entry = adw::PasswordEntryRow::builder()
        ...
        .build();
    password_entry.set_visible(auth_selected == 0);
    ...
    key_row.set_visible(auth_selected == 1);

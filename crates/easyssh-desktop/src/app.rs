...
impl EasySshApp {
    fn from_store(store: ConfigStore, inspect_agent: bool) -> Self {
        ...
        let mut diagnostics_state = state::diagnostics::State::default();
        if inspect_agent {
            diagnostics_state.request();
        }
        Self {
            store,
            config,
                ...
                ui.horizontal(|ui| {
                    ...
                    if matches!(
                        ...
                        ui.menu_button(icon::LIST, |ui| {
                            ...
                            if self.config.experimental.git_metadata_sync_ui
                                && ui.button("Git metadata sync").clicked()
                            {
                                self.sync_open = true;
                                ui.close_menu();
                            }
                        });
                        if icon_button(ui, icon::MAGNIFYING_GLASS, "Search commands and hosts")
                            .clicked()
                        {
                    ...
                    if !matches!(
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if self.config.experimental.git_metadata_sync_ui
                                && icon_button(ui, icon::ARROWS_CLOCKWISE, "Git metadata sync")
                                    .clicked()
                            {
                                self.sync_open = true;
                            }
                            ...
                            let agent = if agent_ready { "Agent ready" } else { "Agent" };
                            ui.label(
        ...
        egui::TopBottomPanel::top("sessions")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new("External terminal launches")
                            .small()
                            .weak(),
                    );
    ...
    fn home(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if self.ssh_config_scanned {
                ui.add_space(8.0);
                ui.label(egui::RichText::new("From SSH Config").strong());
                if self.ssh_config_aliases.is_empty() {
                    ui.label("No concrete host aliases found.");
                }
    ...
    fn keys(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if matches!(
                ...
                self.diagnostics_state.request();
            }
    ...
    #[allow(dead_code)]
    fn settings_legacy(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.add_space(22.0);
            let strings = crate::ui::localization::Strings::new(self.config.locale);
            ui.heading(strings.text(crate::ui::localization::Key::Settings));
            let mut changed = false;
            ui.collapsing("Appearance", |ui| { ui.horizontal(|ui| { ui.label("Theme"); changed |= ui.selectable_value(&mut self.config.theme, Theme::System, "System").changed(); changed |= ui.selectable_value(&mut self.config.theme, Theme::Light, "Light").changed(); changed |= ui.selectable_value(&mut self.config.theme, Theme::Dark, "Dark").changed(); }); ui.horizontal(|ui| { ui.label("Density"); changed |= ui.selectable_value(&mut self.config.display_density, DisplayDensity::Compact, "Compact").changed(); changed |= ui.selectable_value(&mut self.config.display_density, DisplayDensity::Comfortable, "Comfortable").changed(); changed |= ui.selectable_value(&mut self.config.display_density, DisplayDensity::Large, "Large").changed(); }); ui.horizontal(|ui| { ui.label("Language"); changed |= ui.selectable_value(&mut self.config.locale, Locale::System, "System").changed(); changed |= ui.selectable_value(&mut self.config.locale, Locale::En, "English").changed(); changed |= ui.selectable_value(&mut self.config.locale, Locale::ZhCn, "简体中文").changed(); }); });
            ...
            ui.collapsing("Experimental", |ui| { changed |= ui.checkbox(&mut self.config.experimental.remote_text_editing, "Remote text editing").changed(); changed |= ui.checkbox(&mut self.config.experimental.image_preview, "Image preview").changed(); changed |= ui.checkbox(&mut self.config.experimental.dual_pane_file_browsing, "Dual-pane file browsing").changed(); changed |= ui.checkbox(&mut self.config.experimental.git_metadata_sync_ui, "Git metadata sync").changed(); if self.config.experimental.git_metadata_sync_ui && ui.button(format!("{} Open sync settings", icon::ARROWS_CLOCKWISE)).clicked() { self.sync_open = true; } });

...
//!
//! 注意: 监控模块需要 `monitoring` feature 才能使用。

// Fallback main when monitoring feature is not enabled
#[cfg(not(feature = "monitoring"))]
fn main() {
    println!("Monitoring example requires the 'monitoring' feature.");
    println!("Run with: cargo run --example monitoring --features monitoring");
}
...
mod monitoring_example {
    ...
    /// 注意: 此函数需要实际 SSH 服务器连接，故标记为允许未使用
    #[allow(dead_code)]
...
// Re-export main for the monitoring feature
#[cfg(feature = "monitoring")]
fn main() {
    monitoring_example::main();
}

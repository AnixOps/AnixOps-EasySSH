...
impl SshSessionManager {
    ...
    fn cleanup_expired_mux(&mut self) {
        ...
        self.mux_pools.retain(|_, pool| {
            if pool.ref_count == 0 && now.duration_since(pool.last_used) > timeout {
                log::info!(
                    "SSH MUX: 清理过期连接池 (idle {}s)",
                    pool.last_used.elapsed().as_secs()
                );
                false
            } else {
                true
    ...
    pub async fn connect(
        ...
        self.connect_with_mux(session_id, host, port, username, password, false)
            .await
    }
        ...
        if use_mux {
            if let Some(pool_item) = self.mux_pools.get_mut(&mux_key) {
                if pool_item.ref_count < self.max_mux_uses {
                    ...
                    self.sessions
                        .insert(session_id.to_string(), pool_item.session.clone());
                    log::info!(
                        "SSH MUX: 复用连接 {} (ref={})",
                        mux_key,
                        pool_item.ref_count
    ...
    pub fn get_mux_stats(&self) -> MuxStats {
        let pools: Vec<MuxPoolInfo> = self
            .mux_pools
            .iter()

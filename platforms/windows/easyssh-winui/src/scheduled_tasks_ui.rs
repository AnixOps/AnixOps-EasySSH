...
impl ScheduledTasksPanel {
    ...
    pub fn ui(&mut self, ui: &mut Ui, scheduler: &mut TaskScheduler, script_library: &ScriptLibrary) -> ScheduledTasksResponse {
        ...
        if self.show_edit_dialog {
            if self.edit_task_form.is_some() {
    ...
    fn render_task_list(&mut self, ui: &mut Ui, scheduler: &mut TaskScheduler, response: &mut ScheduledTasksResponse) {
        let tasks: Vec<_> = scheduler.get_all_tasks()
            .into_iter()
            .filter(|t| {
                // Filter by status
                ...
                if !self.search_query.is_empty() {
                    if !t.name.to_lowercase().contains(&self.search_query.to_lowercase()) {
                        return false;
                    }
                }
        ...
        if !running.is_empty() {
            ui.colored_label(Color32::GREEN, format!("▶ {} task(s) running", running.len()));
            ...
            for handle in running {
                ui.horizontal(|ui| {
                    ...
                    ui.label(format!("(started {})", handle.started_at.format("%H:%M:%S")));
                });
            }
            ui.separator();
        }
    ...
    fn render_task_row(&mut self, ui: &mut Ui, task: &ScheduledTask, response: &mut ScheduledTasksResponse) {
        ...
        Frame::group(ui.style())
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    // Task name
                    let name_color = if task.enabled { Color32::WHITE } else { Color32::GRAY };
                ...
                // Task details
                ui.horizontal(|ui| {
                    ui.label(RichText::new(&task.schedule_description).size(12.0).color(Color32::LIGHT_GRAY));
                    ui.label("|");
                    ui.label(RichText::new(format!("Workflow: {}", task.workflow_id)).size(12.0));
                    if !task.target_servers.is_empty() {
                        ui.label("|");
                        ui.label(RichText::new(format!("{} servers", task.target_servers.len())).size(12.0));
                    }
                });
                ...
                ui.horizontal(|ui| {
                    if let Some(next) = task.next_run {
                        ui.label(RichText::new(format!("Next: {}", next.format("%Y-%m-%d %H:%M"))).size(11.0));
                    }
                    if let Some(last) = task.last_run {
                        ui.label("|");
                        ui.label(RichText::new(format!("Last: {}", last.format("%Y-%m-%d %H:%M"))).size(11.0));
                    }
    ...
    fn render_create_dialog(&mut self, ui: &mut Ui, _scheduler: &mut TaskScheduler, script_library: &ScriptLibrary, response: &mut ScheduledTasksResponse) {
        ...
        egui::Window::new("Create Scheduled Task")
            ...
            .show(ui.ctx(), |ui| {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    ui.add(egui::Slider::new(&mut self.new_task_form.timeout_minutes, 1..=120));
    ...
    fn render_edit_dialog(&mut self, ui: &mut Ui, _scheduler: &mut TaskScheduler, response: &mut ScheduledTasksResponse) {

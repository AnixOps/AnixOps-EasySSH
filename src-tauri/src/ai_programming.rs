...
pub fn debug_test_ssh() -> Result<DebugTestReport, String> {
    ...
    if !has_none { passed += 1; } else { failed += 1; }
...
pub fn debug_test_pro() -> Result<DebugTestReport, String> {
    ...
    match crate::pro::create_team("Test Team", "owner123") {
        Ok(team) => {
            let valid = !team.id.is_empty() && team.name == "Test Team" && team.owner_id == "owner123";
            results.push(DebugTestResult {
                name: test_name.to_string(),
    ...
    results.push(DebugTestResult {
        ...
        details: Some(format!("Owner.can_manage_members={}, Viewer.can_manage_members={}", owner_can_manage, !viewer_cannot)),
    });
    if both_correct { passed += 1; } else { failed += 1; }

    // 测试3: 审计日志创建
    let test_name = "pro_create_audit_log";
    let log = crate::pro::create_audit_log(
        "team1", "user1", "testuser",
        crate::pro::AuditAction::ServerConnect,
        "server", "srv123", "测试连接",
        Some("127.0.0.1"),
    );
    let valid = log.team_id == "team1" && log.user_id == "user1"
        && matches!(log.action, crate::pro::AuditAction::ServerConnect);
    results.push(DebugTestResult {
        name: test_name.to_string(),
        category: "pro".to_string(),
        passed: valid,
        message: if valid { "审计日志创建成功".to_string() } else { "审计日志数据无效".to_string() },
        details: None,
    });
    if valid { passed += 1; } else { failed += 1; }
...
pub fn debug_test_terminal() -> Result<DebugTestReport, String> {
    ...
    // 构建SSH命令测试 (所有平台)
    let cmd = crate::terminal::build_ssh_command("192.168.1.1", 22, "user", "password");
    let valid = cmd.contains("user@192.168.1.1") && cmd.contains("-p 22");
    results.push(DebugTestResult {
        name: test_name.to_string(),
        category: "terminal".to_string(),
        passed: valid,
        message: if valid { format!("SSH命令构建成功: {}", cmd) } else { "SSH命令格式错误".to_string() },
        details: None,
    });
    if valid { passed += 1; } else { failed += 1; }
...
pub fn debug_quick_check() -> Result<DebugTestReport, String> {
    ...
    match ai_health_check() {
        Ok(h) => {
            ...
            if h.status == "ok" { passed += 1; } else { failed += 1; }
        }
        Err(e) => {
            results.push(DebugTestResult {
                name: "ai_health_check".to_string(),
...
pub async fn git_log(count: usize) -> Result<Vec<GitCommit>, String> {
    let output = tokio::process::Command::new("git")
        .args(["log", &format!("--max-count={}", count), "--pretty=format:%H|%h|%s|%an|%ai"])
        .current_dir(".")
        .output()
        .await
...
pub async fn edit_file(
    ...
    if !content.contains(&old_string) {
        return Ok(EditResult {
            success: false,
            message: format!("未找到匹配的内容: {}", &old_string[..old_string.len().min(50)]),
            old_content: None,
        });
    }
...
pub fn set_context(key: String, value: String) -> Result<(), String> {
    let mut ctx = AI_CONTEXT.lock().map_err(|e| format!("获取上下文锁失败: {}", e))?;
...
pub fn get_context(key: String) -> Result<Option<String>, String> {
    let ctx = AI_CONTEXT.lock().map_err(|e| format!("获取上下文锁失败: {}", e))?;

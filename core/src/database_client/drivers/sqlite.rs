...
impl SqliteDriver {
    ...
    fn map_row_to_cells(&self, row: &Row, column_count: usize) -> Result<Vec<QueryCell>, rusqlite::Error> {
...
impl DatabaseDriver for SqliteDriver {
    ...
    async fn execute_query(&self, query: &str) -> Result<QueryResult, DatabaseError> {
        ...
        let row_iter = stmt.query_map([], |row| {
            self.map_row_to_cells(row, column_count)
        }).map_err(|e| DatabaseError::QueryError(e.to_string()))?;
    ...
    async fn get_tables(&self) -> Result<Vec<TableInfo>, DatabaseError> {
        ...
        for row in rows {
            ...
            let row_count: Option<u64> = conn.query_row(&count_query, [], |r| {
    ...
    async fn get_table_info(&self, table_name: &str) -> Result<TableDetail, DatabaseError> {
        ...
        let columns_rows = columns_stmt.query_map([table_name], |row| {
        ...
        // Get indexes
        let mut indexes_stmt = conn.prepare(
            "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name=?"
        ).map_err(|e| DatabaseError::SchemaError(e.to_string()))?;

        let index_names: Vec<String> = indexes_stmt.query_map([table_name], |row| {
            row.get::<_, String>(0)
        }).map_err(|e| DatabaseError::SchemaError(e.to_string()))?
            .filter_map(|r| r.ok())
            .collect();
        ...
        for index_name in index_names {
            let mut idx_stmt = conn.prepare("PRAGMA index_info(?)")
                .map_err(|e| DatabaseError::SchemaError(e.to_string()))?;

            let idx_cols: Vec<String> = idx_stmt.query_map([&index_name], |row| {
                row.get::<_, String>(2)
            }).map_err(|e| DatabaseError::SchemaError(e.to_string()))?
                .filter_map(|r| r.ok())
                .collect();
        ...
        // Get foreign keys
        let mut fk_stmt = conn.prepare("PRAGMA foreign_key_list(?)")
            .map_err(|e| DatabaseError::SchemaError(e.to_string()))?;

        let fk_rows = fk_stmt.query_map([table_name], |row| {
    ...
    async fn get_performance_metrics(&self) -> Result<PerformanceMetrics, DatabaseError> {
        ...
        Ok(PerformanceMetrics {
            ...
            additional_metrics: [
                ...
                ("database_size_bytes".to_string(), (page_count * page_size) as f64),
            ].into_iter().collect(),
        })
    }
    ...
    async fn cancel(&self) -> Result<(), DatabaseError> {
        ...
        Err(DatabaseError::Unknown("Query cancellation not supported for SQLite".to_string()))
    }
